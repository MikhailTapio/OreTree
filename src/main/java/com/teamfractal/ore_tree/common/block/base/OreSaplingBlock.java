...
import static com.teamfractal.ore_tree.common.block.action.OreTreePlacement.consumeCatalyst;
import static com.teamfractal.ore_tree.common.block.action.OreTreePlacement.tryPlace;
...
public class OreSaplingBlock extends PlantBlock {
    ...
    public OreSaplingBlock(Item catalystItem, Tag<Block> rootable, double randomTickValue, double randomCatalyzeValue, Identifier treeNBTLocation) {
        super(Settings.of(Material.PLANT).ticksRandomly().noCollision().sounds(BlockSoundGroup.GRASS).breakInstantly());
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if(player.getStackInHand(hand).getItem() == catalystItem){
            if (!(i >= 3)) {
                ...
                consumeCatalyst(player, catalystItem.getDefaultStack());
